...
coverage: vendor ## Run coverage unit tests
	docker run -v $(shell pwd):/app -w /app composer:latest bin/phpunit --coverage-html --coverage-text

test: tu tf ## Run unit and functional tests

tu: vendor ## Run unit tests
	docker run -v $(shell pwd):/app -w /app composer:latest bin/phpunit --exclude-group functional
